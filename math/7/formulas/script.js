...
const EXPLANATIONS = {
    ...
    factor_trinomial_2var_deg4: m => {
        ...
        const a2 = term(m.a * m.a, "x⁴");
        const b2 = term(m.b * m.b, "y²");
        const twoab = term(2 * m.a * m.b, "x²y");
        return `Слагаемые даны не по порядку, как в формуле, из-за этого легко ошибиться. ` +
               `Сначала переставим слагаемые как в формуле: ${a2} ${m.sign} ${twoab} + ${b2}, ` +
               `далее по формуле получаем (${aStr} ${m.sign} ${bStr})².`;
    }
};
